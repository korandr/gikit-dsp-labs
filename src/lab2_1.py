...
n = np.arange(0, 128)      # Массив отсчётов
...
def build_plots(constant_component: bool) -> None:
    ...
    plt.stem(n, A, linefmt='g-', markerfmt='go', basefmt='r-')
    plt.title('Амплитудный спектр', fontsize=11, fontweight='bold')
    plt.xlabel('Номер спектрального отсчета (k)')
